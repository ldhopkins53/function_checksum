...
int main(int argc, char **argv)
{
    ...
    Elf64_Shdr *elf_shdr_base = reinterpret_cast<Elf64_Shdr*>(&data[elf_header->e_shoff]);
    char *string_table = &data[(elf_shdr_base + elf_header->e_shstrndx)->sh_offset];
    ...
    {
        std::string current_section_name = &string_table[(elf_shdr_base+i)->sh_name];
    ...
    for (auto i = 0; i < elf_header->e_shnum; ++i)
    {
        ...
        if (current_section_name == ".main_function_crc")
        {
            ...
            main_function_size = reinterpret_cast<int*>(&data[(elf_shdr_base+i)->sh_offset]);
            break;
        }
    }
